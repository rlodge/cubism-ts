...
var events = [
  {
    //Create the New event notification
    name: 'New Event!',
    value: '20',
  },
]
events.push({
  //Create the default event notification
  name: 'No Events',
  value: '0.0',
})
events.push({
...
var context = cubism
  .context() // set the cubism context
...
d3.select('#example1').call(function (div) {
  ...
  const axisDiv = div.append('div').attr('class', 'axis')

  context.axis().orient('top').render(axisDiv)

  RunData()

  const r = div.append('div').attr('class', 'rule')
    ...
    while (last < stop) {
      ...
      for (var j = 0; j < Users.length; j++) {
        ...
        if (Users[j].name == name) {
          //If the user name in the loop matches the current user name
          if (Users[j].newevent) {
            //If the user has a new event
...
function addUser() {
  //Add a user function (executed by pressing add user button)
...
function RefreshEvents() {
  ...
  for (i = 2; i < events.length; i++) {
    $('#EventList').append(
      '<option value="' + i + '">' + events[i].name + '</option>'
    )
  }
}
//////////////////////////////////////////////////
